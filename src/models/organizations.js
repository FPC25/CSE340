...
const getAllOrganizations = async() => {
  try {
    const query = `
        SELECT 
        organization_id, orgname, description, contact_email, logo_filename
...
const getOrganizationDetails = async (organizationId) => {
    const query = `
      SELECT organization_id, orgname, description, contact_email, logo_filename
      FROM organization
...
const updateOrganization = async (organizationId, name, description, contactEmail, logoFilename) => {
  const query = `
    UPDATE organization
    SET orgname = $1, description = $2, contact_email = $3, logo_filename = $4
